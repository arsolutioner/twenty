...
- `CoreApiClient` (for workspace data via `/graphql`) is auto-generated by `yarn twenty app:dev` and stored in `node_modules/twenty-sdk/generated`. `MetadataApiClient` (for workspace configuration and file uploads via `/metadata`) ships pre-built with the SDK. Both are available via `import { CoreApiClient, MetadataApiClient } from 'twenty-sdk/clients'`.
